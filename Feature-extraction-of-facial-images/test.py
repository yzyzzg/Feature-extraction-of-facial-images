...
def get_featurs(model, test_list, batch_size=10):
    ...
    for i, img_path in enumerate(test_list):
        ...
        # 确保 `image` 不为 None 且维度正确
        if image is not None and len(image.shape) == 4:
            if images is None:
                images = image
            else:
                # 确保 `images` 和 `image` 的维度一致
                if images.shape[1:] != image.shape[1:]:
                    print(f"Dimension mismatch: images {images.shape}, image {image.shape}")
                    continue
                images = np.concatenate((images, image), axis=0)

        # 当达到 batch_size 或是最后一张图时，进行特征提取
        if images is not None and (images.shape[0] % batch_size == 0 or i == len(test_list) - 1):
            cnt += 1
            ...
            if len(output.shape) == 2:
                ...
                print(f"Output dimension error: {output.shape}")
